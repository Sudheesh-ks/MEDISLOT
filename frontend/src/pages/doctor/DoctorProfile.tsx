...
const DoctorProfile = () => {
  ...
  const input = "bg-transparent ring-1 ring-white/10 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500";
  ...
  /*  status gates (same copy, colours tweaked)  */
  ...
  return profileData && form ? (
    ...
    >
      {/* ───────── PHOTO ───────── */}
      <motion.div whileHover={{ scale: 1.04 }} className={`${glass} p-6 rounded-3xl flex justify-center items-center`}>
        {isEdit ? (
          <label htmlFor="doc-img" className="cursor-pointer relative">
            ...
            <input id="doc-img" type="file" hidden onChange={e => setImage(e.target.files?.[0] || null)} />
          </label>
        ) : (
          <img src={profileData.image} className="w-full bg-primary rounded-xl object-cover ring-1 ring-white/10" />
        )}
      </motion.div>
      ...
      >
        ...
        <div>
          {isEdit ? (
            <input name="name" value={form.name} onChange={onField} className={`text-3xl font-semibold ${input}`} />
          ) : (
          ...
          <div className="mt-1 text-slate-400">
            {isEdit ? (
              <>
                <input name="degree" value={form.degree} onChange={onField} className={`${input} mr-2`} placeholder="Degree" />
                -
                <input name="speciality" value={form.speciality} onChange={onField} className={`${input} ml-2`} placeholder="Speciality" />
              </>
            ) : (
          ...
          <div className="text-sm text-slate-500">
            Experience:&nbsp;
            {isEdit ? (
              <input name="experience" value={form.experience} onChange={onField} className={`${input} w-20`} />
            ) : (
        ...
        <div>
          ...
          {isEdit ? (
            <textarea name="about" value={form.about} onChange={onField} className={`${input} w-full mt-1`} />
          ) : (
        ...
        <div className="flex items-center gap-3">
          ...
          {isEdit ? (
            ...
            <span className="text-slate-100">{currencySymbol}{profileData.fees}</span>
          )}
        </div>
        ...
        <div>
          ...
          {isEdit ? (
            <>
              <input value={form.address.line1} onChange={e => setAddr("line1", e.target.value)} className={`${input} w-full mt-1`} />
              <input value={form.address.line2} onChange={e => setAddr("line2", e.target.value)} className={`${input} w-full mt-2`} />
            </>
          ) : (
        ...
        <div className="flex items-center gap-3">
          ...
          {isEdit ? (
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${avail ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-600/40 text-slate-400"}`}>
        ...
        <motion.button
          ...
          onClick={isEdit ? save : () => { setEdit(true); setForm(profileData); }}
        >
