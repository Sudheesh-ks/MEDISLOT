...
const DocChatPage: React.FC = () => {
  ...
  const handleSendMessage = (e: React.FormEvent | React.MouseEvent) => {
    ...
    setNewMessage("");
    socket.emit("stopTyping", { chatId, senderId: doctorId });
  };
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
        {messages.map((msg) => {
          ...
          return (
            <div key={msg._id} className={`flex ${isDoctor ? "justify-end" : "justify-start"}`}>
              <div
      ...
      >
        <input
          ...
          onFocus={() => socket.emit("typing", { chatId, senderId: doctorId })}
          onBlur={() => socket.emit("stopTyping", { chatId, senderId: doctorId })}
        />
        <button
          type="submit"
