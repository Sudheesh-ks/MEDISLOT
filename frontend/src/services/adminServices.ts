...
export const getAllDoctorsAPI = async (token: string) => {
  return await api.post('/api/admin/all-doctors', {}, {
...
export const changeAvailabilityAPI = async (docId: string, aToken: string) => {
  return await api.post('/api/admin/change-availability', { docId }, {
    headers: { aToken },
  });
};
...
export const toggleUserBlockAPI = async (userId: string, aToken: string) => {
  return await api.post('/api/admin/toggle-user-block', { userId }, {
    headers: { aToken },
  });
};